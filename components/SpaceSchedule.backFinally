...
const SpaceSchedule = ({ onClose }) => {
  ...
  const [isConfiguringSpecificDate, setIsConfiguringSpecificDate] = useState(false);
  ...
  const debugBlockedSlots = () => {
    ...
    console.log(' Lista completa de slots bloqueados:');
    ...
    // Verificar slots para el día seleccionado
    if (selectedDay) {
    ...
    console.log(' Estado actual de weekDaysWithDates:');
  ...
  const debugSelectedDate = () => {
    ...
    console.log(`Día seleccionado: ${selectedDay.name} (${selectedDay.id})`);
    console.log(`Fecha del día seleccionado: ${currentDayDate}`);
  ...
  const renderTimeSlot = (slot, day) => {
    ...
    console.log('Verificando disponibilidad para día:', dayId, 'hora:', hourId);
    console.log('Configuración de disponibilidad actual:', availabilitySettings);
  ...
  const debugWeekDays = () => {
    ...
    Object.keys(blockedSlotsByDate).forEach(dateKey => {
      ...
      // Verificar si esta fecha corresponde a algún día de la semana
      const dayWithThisDate = weekDaysWithDates.find(day => day.date === dateKey);
      if (dayWithThisDate) {
        console.log(`    ✅ Corresponde al día: ${dayWithThisDate.name} (${dayWithThisDate.id})`);
      } else {
        console.log(`    ❌ No corresponde a ningún día de esta semana`);
      }
    });
  };

  // Función para depurar profundamente el problema de fechas
  const debugDateIssue = () => {
    ...
    // Mostrar solo la información esencial en formato limpio
    Alert.alert(
      'Depuración de Fechas',
      `Fecha actual:\n${formattedDate}\n` +
      `Día: ${localDate.getDate()}, Mes: ${localDate.getMonth() + 1}, Año: ${localDate.getFullYear()}\n` +
      `Día de la semana: ${getDayName(localDate.getDay())}\n\n` +
      `Slots bloqueados:\n` +
  ...
  // Función para mostrar más detalles de depuración
  const showMoreDebugInfo = () => {
    ...
    // Recopilar información detallada sobre los slots bloqueados
    let detailedInfo = '';
    
    // Mostrar información de la fecha actual
    detailedInfo += `Fecha actual: ${formattedDate}\n`;
    detailedInfo += `Día de la semana: ${getDayName(localDate.getDay())}\n\n`;
    ...
    if (!blockedSlotsByDate || Object.keys(blockedSlotsByDate).length === 0) {
      ...
      Object.keys(blockedSlotsByDate).forEach(dateKey => {
        ...
        if (dayWithThisDate) {
          ...
          detailedInfo += 'No corresponde a ningún día de esta semana\n';
        }
        
        // Mostrar detalles de los slots
        blockedSlotsByDate[dateKey].forEach((slot, index) => {
          detailedInfo += `  Slot ${index + 1}: hora=${slot.hour}, día=${slot.day}\n`;
        });
      });
    }
    
    // Mostrar la información detallada en una alerta
    Alert.alert(
  ...
  // Función para limpiar slots duplicados
  const cleanupDuplicateSlots = () => {
    ...
    // Crear un mapa para rastrear slots únicos por hora
  ...
  // Función para verificar la fecha actual
